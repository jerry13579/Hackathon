...
for i in dept:
    ...
    try:
        ...
        df = pd.DataFrame(columns=['CRN', 'COURSE', 'SECTION', 'DAY', 'START TIME', 'END TIME', 'FIRST NAME', 'LAST NAME', 'LOCATION'])
        ...
        for row in table.tbody.find_all('tr'):
            ...
            if columns:
                try:
                    ...
                    lastName += name[spaceIndex+1:len(name)]
                    if firstName[spaceIndex-1] == ',':
                ...
                df = df._append({'CRN': crn, 'COURSE': course, 'SECTION': section,'DAY': days,
                                 'START TIME': startTime, 'END TIME': endTime, 'FIRST NAME': firstName, 'LAST NAME': lastName,
